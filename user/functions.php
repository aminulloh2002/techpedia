...
//ambil data dari tiap elemen dari dalam form
    ...
    // query insert data
    $query = "INSERT INTO barang VALUES ('','$mrk','$tp','$hrg','$psr','$rm','$grf','$pny','$gb')";
...
function registrasi($data){
  ...
  $fname = strtolower(stripslashes($data["fname"]));
  $lname = strtolower(stripslashes($data["lname"]));
  ...
  // tambahkan userbaru ke database
   mysqli_query($conn, "INSERT INTO user VALUES('', '$fname','$lname','$telp','$email','$password')");
...
function admubahpass($data){
  ...
  if($passwordlama != $pass){
   ...
   $passwordbaru = password_hash($passwordbaru, PASSWORD_DEFAULT);
    ...
    // tambahkan userbaru ke database
   mysqli_query($conn,"UPDATE user SET password = '$passwordbaru' WHERE id_user='$id'");
